...
void Button::draw_button() {
    ...
    if (check_hover()) {
        if (button_hover_info["x_pos"].get<float>() == 0 || button_hover_info["y_pos"].get<float>() == 0) {
            return;
        };
        ...
        // Not on hover
        if (button_info["x_pos"].get<float>() == 0 || button_info["y_pos"].get<float>() == 0) {
            return;
        };

        // Convert coodinates from the center back to the corner for actual drawing. This can actually be 0
        int calibrated_x = button_info["x_pos"].get<float>() - button_info["width"].get<float>()/2;
        ...
        int text_y = calibrated_y + (button_info["height"].get<float>() - text_height)/2;
        Vector2 text_position = {static_cast<float>(text_x), static_cast<float>(text_y)};
